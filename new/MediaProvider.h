...
public:
    ...
    //gets the internal media container
    const std::map<std::string, T>& getMediaContainer() const {return mMediaContainer;};
private:
    ///variables
    //textures
...
bool MediaProvider<T>::loadMedia(const std::string& key, const std::string& path) {
    ...
    if(tmp.loadFromFile(path)) {
        //add it the the container
        mMediaContainer[key] = std::move(tmp);
        //success
        return true;
    }
    else {
